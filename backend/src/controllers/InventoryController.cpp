...
void InventoryController::registerRoutes(httplib::Server& server)
{
    ...
    server.Post("/api/inventory/inbound", [](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            if (inbound.createdBy <= 0) {
                inbound.createdBy = mis::context::currentContext.userId;
            }
            ...
            if (inbound.lines.empty() && body.contains("skuCode")) {
                models::InboundLine line;
                std::string skuStr = body.value("skuCode", "");
                if (skuStr.empty() || !std::all_of(skuStr.begin(), skuStr.end(), ::isdigit)) {
                    res.status = 400;
                    res.set_content(fail(-1, "商品编号/SKU 必须为数值型 ID（例如 1001, 1002）"), "application/json");
                    return;
                }
                line.productId = std::stoi(skuStr);
    ...
    server.Post(R"(/api/inventory/inbound/(\d+)/submit)", [](const httplib::Request& req, httplib::Response& res) {
        try {
            int id = std::stoi(req.matches[1]);
            services::makeInventoryService().submitInbound(id, mis::context::currentContext.userId);
            res.status = 200;
    ...
    server.Post(R"(/api/inventory/inbound/(\d+)/cancel)", [](const httplib::Request& req, httplib::Response& res) {
        try {
            int id = std::stoi(req.matches[1]);
            services::makeInventoryService().cancelInbound(id, mis::context::currentContext.userId);
            res.status = 200;
    ...
    server.Post(R"(/api/inventory/inbound/lines/(\d+)/receive)", [](const httplib::Request& req, httplib::Response& res) {
        try {
            ...
            services::makeInventoryService().receiveLine(lineId, quantity, mis::context::currentContext.userId);
            res.status = 200;
    ...
    server.Get("/api/inventory/dashboard", [](const httplib::Request&, httplib::Response& res) {
        try {
            ...
            for (const auto& p : stats.trend) {
                trend.push_back({{"date", p.first}, {"quantity", p.second}});
            }
